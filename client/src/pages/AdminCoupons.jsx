...
import { dashboardInlineToggleClass } from "../utils/dashboardFormStyles";
...
const toImageUrl = (value) => {
  ...
  return resolved.startsWith("/") ? `${baseRoot}${resolved}` : `${baseRoot}/${resolved}`;
};
...
const SelectionPanel = ({
  ...
  <div className="rounded-[26px] border border-slate-200 bg-slate-50/90 px-4 pb-4 pt-3.5 shadow-[0_16px_36px_rgba(8,23,68,0.03)] sm:px-5 sm:pb-5 sm:pt-4">
    ...
    <div className={`grid max-h-[360px] gap-3 overflow-y-auto pr-1 ${columnsClassName}`}>
      {items.length === 0 ? (
        ...
        items.map((item) => {
          ...
          return (
            <label
              key={itemKey}
              className={`block cursor-pointer rounded-[22px] border px-4 py-4 text-sm transition ${
                checked
                  ? "border-slate-950 bg-white text-slate-950 shadow-[0_18px_32px_rgba(8,23,68,0.08)]"
            ...
            >
              <input
                ...
                className="sr-only"
              />
              <div className="min-w-0">{renderContent(item, checked)}</div>
            </label>
          );
        })
      )}
...
const AdminCoupons = () => {
  ...
  }, [isAdmin, isVendor, fetchCoupons, fetchCategories, fetchProducts]);
  ...
  const handleEdit = (coupon) => {
    ...
    const nextTargetCategories = Array.isArray(coupon.targetCategories)
      ? coupon.targetCategories.map((entry) => entry?._id || entry).filter(Boolean)
      : [];
    const nextTargetProducts = Array.isArray(coupon.targetProducts)
      ? coupon.targetProducts.map((entry) => entry?._id || entry).filter(Boolean)
      : [];
    ...
    setCategoryNamesEnabled(nextTargetCategories.length > 0 || nextTargetProducts.length > 0);
  ...
  const getCouponTargetSummary = (coupon) => {
    if (String(coupon?.applicabilityMode || "global").toLowerCase() !== "targeted") {
      return "Applies to the full catalog";
    }

    const parts = [];
    if (Array.isArray(coupon?.targetCategoryTypes) && coupon.targetCategoryTypes.length > 0) {
      parts.push(`${coupon.targetCategoryTypes.length} category type(s)`);
    }
    if (Array.isArray(coupon?.targetCategories) && coupon.targetCategories.length > 0) {
      parts.push(`${coupon.targetCategories.length} category name(s)`);
    }
    if (Array.isArray(coupon?.targetProducts) && coupon.targetProducts.length > 0) {
  ...
  const normalizedSelectedCategoryTypes = useMemo(
    () => form.targetCategoryTypes.map((entry) => String(entry || "").trim().toLowerCase()),
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter((category) => {
      const categoryType = String(category?.type || "General").trim().toLowerCase();
      if (
  ...
  }, [form.targetCategories.length, productSearch, products, selectedCategoryIdSet]);
  ...
  useEffect(() => {
    if (form.targetCategoryTypes.length === 0) {
      setForm((prev) => {
        if (prev.targetCategories.length === 0 && prev.targetProducts.length === 0) {
          return prev;
        }
        return {
          ...prev,
    ...
    const allowedCategoryIds = new Set(
      categories
        .filter((category) =>
          allowedTypes.has(String(category?.type || "General").trim().toLowerCase()),
        )
    ...
    setForm((prev) => {
      ...
      const nextProducts = prev.targetProducts.filter((entry) => {
        const product = products.find(
          (productItem) => String(productItem?._id || "") === String(entry || ""),
        );
  ...
  }, [categories, normalizedSelectedCategoryTypes, products, form.targetCategoryTypes.length]);
  ...
  useEffect(() => {
    ...
    const allowedCategoryIds = new Set(form.targetCategories.map((entry) => String(entry || "")));
    setForm((prev) => {
      const nextProducts = prev.targetProducts.filter((entry) => {
        const product = products.find(
          (productItem) => String(productItem?._id || "") === String(entry || ""),
      ...
      <div className="space-y-8">
            ...
            <div className="space-y-8">
              <section className="space-y-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <h2 className="text-lg font-semibold text-slate-950">Coupon Setup</h2>
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          <label className="order-3 md:col-span-2 lg:col-span-3">
            ...
            >
              ...
              <option value="targeted">Targeted Coupon</option>
            </select>
          </label>
            ...
            <div className="order-4 space-y-5 md:col-span-2 lg:col-span-3">
              <div className="rounded-[28px] border border-slate-200 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] px-5 pb-5 pt-4 shadow-[0_18px_44px_rgba(8,23,68,0.04)] sm:px-6 sm:pb-6 sm:pt-5">
                <div className="text-left">
                  <h2 className="text-lg font-semibold text-slate-950">Target Catalog</h2>
                  <p className="mt-1 text-sm leading-6 text-slate-500">
                    Build the coupon chain step by step. Start from category type,
                    activate category names when needed, then activate products under the
                    chosen categories.
                  </p>
                </div>
                <div className="mt-4 space-y-4">
                    <SelectionPanel
                      ...
                      onToggle={(value) => toggleSelection("targetCategoryTypes", value)}
                      onClear={() =>
                        setForm((prev) => ({
                          ...prev,
                          targetCategoryTypes: [],
                      ...
                      renderContent={(item) => (
                        <div className="min-w-0">
                          <p className="truncate font-semibold text-slate-950">{item}</p>
                          <p className="mt-1 text-xs text-slate-500">Catalog grouping</p>
                        </div>
                      )}
                      selectedLabel="selected"
                    ...
                    >
                      <div className="flex flex-col items-start gap-3">
                        <div>
                          <p className="text-sm font-semibold text-slate-950">Category Name Layer</p>
                          <p className="mt-1 text-xs leading-5 text-slate-500">
                            After selecting category type, activate this layer to choose the
                            matching category names only.
                          </p>
                        </div>
                        <button
                          type="button"
                          ...
                          className={`${dashboardInlineToggleClass(categoryNamesEnabled)} disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400`}
                        >
                          {categoryNamesEnabled ? "Category Names Active" : "Activate Category Names"}
                        </button>
                      </div>
                    </div>
                    ...
                    {categoryNamesEnabled ? (
                      <SelectionPanel
                        ...
                        onToggle={(value) => toggleSelection("targetCategories", value)}
                        onClear={() =>
                          setForm((prev) => ({ ...prev, targetCategories: [], targetProducts: [] }))
                        }
                    ...
                    {categoryNamesEnabled ? (
                      ...
                      >
                        <div className="flex flex-col items-start gap-3">
                          <div>
                            <p className="text-sm font-semibold text-slate-950">Product Layer</p>
                            <p className="mt-1 text-xs leading-5 text-slate-500">
                              Activate products when you want the coupon to focus on selected
                              products from the chosen category names.
                            </p>
                          </div>
                          <button
                            type="button"
                            ...
                            onClick={() => {
                              setProductsEnabled((prev) => {
                                const nextValue = !prev;
                                if (!nextValue) {
                                  setForm((current) => ({ ...current, targetProducts: [] }));
                                }
                                return nextValue;
                              });
                            }}
                            className={`${dashboardInlineToggleClass(productsEnabled)} disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400`}
                          >
                            {productsEnabled ? "Products Active" : "Activate Products"}
                          </button>
                        </div>
                      </div>
                    ) : null}
                    ...
                    {productsEnabled ? (
                      <SelectionPanel
                        ...
                        onToggle={(value) => toggleSelection("targetProducts", value)}
                        onClear={() =>
                          setForm((prev) => ({ ...prev, targetProducts: [] }))
                        }
                          ...
                          return (
                            <div className="flex min-w-0 items-start gap-3">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="mt-1 text-xs text-slate-500">
                                  {item?.brand || item?.category?.name || "Catalog product"}
                                </p>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {form.targetCategoryTypes.map((entry) => (
                            <button
                              key={entry}
                              type="button"
                              onClick={() => toggleSelection("targetCategoryTypes", entry)}
                          ...
                          {selectedCategories.map((category) => (
                            <button
                              key={category._id}
                              type="button"
                              onClick={() => toggleSelection("targetCategories", String(category._id))}
                              className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:border-black"
                            >
                          ...
                          {selectedProducts.map((product) => (
                            <button
                              key={product._id}
                              type="button"
                              onClick={() => toggleSelection("targetProducts", String(product._id))}
                              className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:border-black"
                            >
          ...
          {form.offerType === "combo" ? (
            <div className="space-y-5">
              <div>
                <h2 className="text-lg font-semibold text-slate-950">Combo Products</h2>
                <p className="mt-1 text-sm text-slate-500">
                  Pick the products that must be present before the combo coupon applies.
                </p>
              </div>
              ...
              <SelectionPanel
                ...
                onToggle={(value) => toggleSelection("requiredProducts", value)}
                onClear={() =>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedComboProducts.map((product) => (
                      <button
                        key={product._id}
                        type="button"
                        onClick={() => toggleSelection("requiredProducts", String(product._id))}
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-slate-200 pt-6">
          <div className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700">
            {editingId ? "Editing existing coupon" : "Ready to publish a new coupon"}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              type="submit"
              disabled={saving}
              className="rounded-full bg-[linear-gradient(135deg,#081744_0%,#202d5a_100%)] px-6 py-3 text-sm font-bold text-white shadow-lg shadow-slate-900/15 transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-60"
            >
        ...
        <section className="space-y-6">
          ...
          {loading ? (
            <div className={`${sectionClassName} text-sm text-slate-500`}>Loading coupons...</div>
          ) : coupons.length === 0 ? (
            <div className={sectionClassName}>
              <div className="rounded-[24px] border border-slate-200 bg-slate-50 p-10 text-center">
                ...
                <h3 className="text-2xl font-bold text-slate-950">No Active Coupons</h3>
                <p className="mx-auto mt-3 max-w-xs text-sm leading-6 text-slate-500">
                  Create your first coupon above and bring targeted offers into the storefront.
                </p>
              </div>
            </div>
          ) : (
