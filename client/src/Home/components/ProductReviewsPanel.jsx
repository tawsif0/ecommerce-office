...
const getReviewerDisplayName = (review = {}) =>
  String(review?.user?.name || review?.reviewerName || "Customer").trim() || "Customer";
...
const ProductReviewsPanel = ({
  ...
  const sortedReviews = useMemo(() => {
    ...
    if (reviewSortOrder === "lengthy") {
      return nextReviews.sort((a, b) => {
        const commentDiff =
          String(b?.comment || "").trim().length - String(a?.comment || "").trim().length;
  ...
  const normalizedReviewSummary = useMemo(() => {
    ...
    const fallbackAverage =
      fallbackCount > 0
        ? reviewList.reduce((sum, review) => sum + Number(review?.rating || 0), 0) / fallbackCount
        : 0;
  ...
  const handleShareReview = async (review = {}) => {
    const shareUrl =
      typeof window !== "undefined"
        ...
        : "";
    const shareTitle = String(review?.title || productTitle || "Product review").trim();

    try {
      if (navigator.share) {
        await navigator.share({
          title: shareTitle,
          text: String(review?.comment || "").trim().slice(0, 180),
          url: shareUrl,
        });
        return;
      }
    ...
    <div className="w-full overflow-hidden pb-4 pt-1 sm:pt-2">
      <header className="mb-8 flex flex-col gap-5 md:mb-12 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm leading-7 text-zinc-500 sm:text-base md:text-lg">
            Read approved customer feedback and share your own experience with this product.
          </p>
        </div>
      ...
      >
        <div className="mb-6 flex flex-col gap-3 sm:mb-8 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          ...
          <div className="w-full rounded-full border border-zinc-200 bg-white px-4 py-2 text-center text-[11px] font-semibold uppercase tracking-[0.16em] text-zinc-500 sm:w-auto sm:text-xs sm:tracking-[0.18em]">
        ...
        <form onSubmit={onSubmitReview} className="grid grid-cols-1 gap-6 sm:gap-8 lg:grid-cols-12 lg:gap-12">
          <div className="space-y-6 lg:col-span-4">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[1, 2, 3, 4, 5].map((star) => {
                  const currentRating = Number(hoverRating ?? reviewForm.rating ?? 0);
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-full transition ${active ? "text-zinc-950" : "text-zinc-300"}`}
                      aria-label={`Rate ${star} star${star > 1 ? "s" : ""}`}
                    >
                      <FaStar
                        className={`h-8 w-8 sm:h-9 sm:w-9 ${
                          active ? "fill-zinc-950 text-zinc-950" : "text-zinc-300"
                        }`}
                      />
                    </button>
                  );
            ...
            <div className="rounded-[1.5rem] border border-zinc-200/70 bg-white px-5 py-5 text-sm leading-7 text-zinc-500 shadow-sm">
              {!isLoggedIn ? (
                <>
                ...
                "Your feedback shapes the quality of this catalog. Keep it precise, useful, and authentic."
              )}
            </div>
          </div>
          ...
          <div className="space-y-5 lg:col-span-8">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(event) => onFieldChange?.("reviewerName", event.target.value)}
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(event) => onFieldChange?.("reviewerEmail", event.target.value)}
                    className={reviewFieldClassName}
            ...
            <div className="flex flex-wrap items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="rounded-xl bg-zinc-950 px-8 py-3.5 text-sm font-bold uppercase tracking-[0.22em] text-white shadow-lg shadow-zinc-950/15 transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {reviewSubmitting
                  ? myReview
                    ? "Updating..."
      ...
      <div className="mb-6 flex flex-col gap-4 sm:mb-8 md:flex-row md:items-center md:justify-between">
        <div>
          <h3 className="text-xl font-bold text-zinc-950 sm:text-2xl">Review Feed</h3>
        ...
        <div className="relative inline-flex w-full items-center md:w-auto">
          <select
            ...
            onChange={(event) => setReviewSortOrder(event.target.value)}
            className="w-full cursor-pointer appearance-none rounded-2xl border border-zinc-200 bg-white px-5 py-3 pr-12 text-sm font-semibold text-zinc-700 shadow-sm outline-none transition hover:border-zinc-400 focus:border-black md:min-w-[220px]"
          >
            <option value="recent">Most Recent</option>
            <option value="highest">Highest Rated</option>
            <option value="lengthy">Lengthy Descriptions</option>
          </select>
          <span className="pointer-events-none absolute right-4 text-zinc-400">
      ...
      <div className="space-y-8">
        {reviewsLoading ? (
          ...
          <div className="rounded-[2rem] border border-dashed border-zinc-300 bg-white px-8 py-10 text-center text-sm text-zinc-500">
            No approved reviews yet. Be the first to submit one for admin approval.
          </div>
        ) : (
