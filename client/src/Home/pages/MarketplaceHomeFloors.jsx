    ...
    >
      ...
      <div className="mt-3 flex min-h-0 flex-1 flex-col">
        ...
        <p className="mt-1 line-clamp-1 text-xs text-gray-500">
          {product?.brand ||
            (typeof product?.category === "object" ? product.category?.name : product?.productType) ||
            "Marketplace product"}
        </p>
...
const MarketplaceHomeFloors = () => {
  ...
  const branding = useMemo(
    () => ({
      storeName: String(settings?.website?.storeName || "E-Commerce").trim() || "E-Commerce",
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setProducts(
          Array.isArray(productResponse.data?.products) ? productResponse.data.products : [],
        );
        setCategories(
  ...
  const dealProducts = useMemo(() => {
    return [...products]
      ...
      .sort((left, right) => {
        const leftGap =
          Number(getPricing(left).previousPrice || 0) - Number(getPricing(left).currentPrice || 0);
        const rightGap =
  ...
  const dealProductCount = useMemo(() => {
    return products.filter(
      (product) => String(product?.priceType || "single").toLowerCase() === "best",
    ).length;
  }, [products]);
  ...
  const storeName = String(branding.storeName || "E-Commerce").trim() || "E-Commerce";
  const highlightsEyebrow =
    String(storefront?.highlightsEyebrow || DEFAULT_STOREFRONT.highlightsEyebrow).trim() ||
    DEFAULT_STOREFRONT.highlightsEyebrow;
  ...
  const highlightsDescription =
    String(
      storefront?.highlightsDescription || DEFAULT_STOREFRONT.highlightsDescription,
    ).trim() || DEFAULT_STOREFRONT.highlightsDescription;
  const flashEyebrow =
    String(storefront?.flashEyebrow || DEFAULT_STOREFRONT.flashEyebrow).trim() ||
    DEFAULT_STOREFRONT.flashEyebrow;
  const flashTitle =
    applyTemplate(storefront?.flashTitle, { storeName }) || DEFAULT_STOREFRONT.flashTitle;
  const flashDescription =
    String(storefront?.flashDescription || DEFAULT_STOREFRONT.flashDescription).trim() ||
    DEFAULT_STOREFRONT.flashDescription;
  const flashPrimaryLabel =
    String(storefront?.flashPrimaryLabel || DEFAULT_STOREFRONT.flashPrimaryLabel).trim() ||
    DEFAULT_STOREFRONT.flashPrimaryLabel;
  ...
  const trustEyebrow =
    String(storefront?.trustEyebrow || DEFAULT_STOREFRONT.trustEyebrow).trim() ||
    DEFAULT_STOREFRONT.trustEyebrow;
  const trustBullets =
    Array.isArray(storefront?.trustBullets) && storefront.trustBullets.length > 0
      ? storefront.trustBullets
      : DEFAULT_STOREFRONT.trustBullets;
  const topCategoriesEyebrow =
    String(
      storefront?.topCategoriesEyebrow || DEFAULT_STOREFRONT.topCategoriesEyebrow,
    ).trim() || DEFAULT_STOREFRONT.topCategoriesEyebrow;
  const dealsEyebrow =
    String(storefront?.dealsEyebrow || DEFAULT_STOREFRONT.dealsEyebrow).trim() ||
    DEFAULT_STOREFRONT.dealsEyebrow;
  const dealsTitle =
    applyTemplate(storefront?.dealsTitle, { storeName }) || DEFAULT_STOREFRONT.dealsTitle;
  const dealsButtonLabel =
    String(storefront?.dealsButtonLabel || DEFAULT_STOREFRONT.dealsButtonLabel).trim() ||
    DEFAULT_STOREFRONT.dealsButtonLabel;
  const categoryFloorEyebrow =
    String(
      storefront?.categoryFloorEyebrow || DEFAULT_STOREFRONT.categoryFloorEyebrow,
    ).trim() || DEFAULT_STOREFRONT.categoryFloorEyebrow;
  const categoryFloorDescription =
    String(
      storefront?.categoryFloorDescription || DEFAULT_STOREFRONT.categoryFloorDescription,
  ...
  const recommendedButtonLabel =
    String(
      storefront?.recommendedButtonLabel || DEFAULT_STOREFRONT.recommendedButtonLabel,
