...
const renderPricingDisplay = (pricing) => {
  ...
  return <span className="text-sm font-bold text-[#2d3435]">{formatMoney(pricing.currentPrice)}</span>;
};
...
const getVendorLabel = (item) =>
  String(item?.vendor?.storeName || "").trim() || "Marketplace";
      ...
      const options = (definition.options || [])
        .map((option) => {
          ...
          const price = usesProductPrice
            ? basePricing?.currentPrice === null || basePricing?.currentPrice === undefined
              ? null
              : Number(basePricing.currentPrice)
            : Number(option?.price);
          const comparePrice = usesProductPrice
            ? basePricing?.previousPrice === null || basePricing?.previousPrice === undefined
              ? null
          ...
          return {
            ...
            colorHex:
              definition.preset === "color" && looksLikeHexColor(option?.colorHex)
...
const CompareProducts = () => {
      ...
      const responses = await Promise.all(
        productIds.map(async (productId) => {
          try {
            ...
            const productData =
              response.data?.product || response.data?.data || response.data || null;
  ...
  const compareCards = React.useMemo(
    () =>
      items.map((item) => {
        const latestProduct = hydratedProducts[String(item?._id || "").trim()] || null;
        const mergedProduct = latestProduct
          ? {
              ...
              vendor: latestProduct.vendor || item.vendor,
              images:
                Array.isArray(latestProduct.images) && latestProduct.images.length
                  ? latestProduct.images
                  : item.images,
              variantDefinitions:
                Array.isArray(latestProduct.variantDefinitions)
                  ? latestProduct.variantDefinitions
                  : item.variantDefinitions || [],
              variations:
                Array.isArray(latestProduct.variations)
        ...
        return {
          ...mergedProduct,
          pricing,
          currentPrice: pricing.isTba ? null : Number(pricing.currentPrice || 0),
  ...
  const comparisonRows = React.useMemo(
    () => [
      {
        label: "Seller",
        value: (item) => getVendorLabel(item),
      },
      {
        label: "Category",
  ...
  const lowestPriceItem = React.useMemo(() => {
    const pricedItems = compareCards.filter((item) => Number.isFinite(item.currentPrice));
    if (!pricedItems.length) return null;
    return [...pricedItems].sort((left, right) => left.currentPrice - right.currentPrice)[0];
  }, [compareCards]);
        ...
        <section className="space-y-3">
          ...
          >
            ...
            {compareCards.map((item) => (
              ...
              >
                {String(item?.title || "").trim().slice(0, 22)}
              </div>
            ))}
          </div>
