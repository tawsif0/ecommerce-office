...
const VendorShippingZones = () => {
  ...
  const fetchZones = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${baseUrl}/shipping/me/zones`, {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${baseUrl}/shipping/me/zones/${editingId}`, payload, {
          headers: getAuthHeaders(),
        });
        toast.success("Shipping zone updated");
      } else {
        await axios.post(`${baseUrl}/shipping/me/zones`, payload, {
          headers: getAuthHeaders(),
        });
  ...
  const confirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await axios.delete(`${baseUrl}/shipping/me/zones/${deleteTarget._id}`, {
        headers: getAuthHeaders(),
      });
  ...
  if (user?.userType !== "vendor") {
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-8 text-center">
        ...
        <p className="text-gray-600">Only vendors can manage vendor shipping zones.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-linear-to-r from-cyan-900 to-black rounded-xl p-6 md:p-8 text-white">
        ...
        <h1 className="text-2xl font-bold">Vendor Shipping Rules</h1>
        <p className="text-cyan-100 mt-1">
          Set your store specific delivery fees and estimated shipping times.
        </p>
      </div>
...
export default VendorShippingZones;
