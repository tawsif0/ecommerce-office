  ...
  const fetchReviews = useCallback(
    async (nextPage = 1) => {
      try {
        ...
        toast.error(error.response?.data?.message || "Failed to load product reviews");
        setReviews([]);
      } finally {
        setLoading(false);
      }
  ...
  if (user?.userType !== "admin") {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-8 text-center">
        ...
        <p className="text-gray-600">Only admins can moderate product reviews.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl bg-linear-to-r from-slate-900 to-black p-6 text-white md:p-8">
        <h1 className="text-2xl font-bold md:text-3xl">Product Review Moderation</h1>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 md:p-6">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center">
          <form onSubmit={handleSearchSubmit} className="flex-1">
            <div className="flex gap-2 rounded-lg border border-gray-200 px-3 py-2">
              <FiSearch className="h-4 w-4 shrink-0 self-center text-gray-500" />
              <input
                value={search}
            ...
            {reviews.map((review) => (
              ...
              >
                <div className="lg:col-span-8">
                  ...
                  <div className="mt-1 flex items-center gap-1">{renderStars(review.rating || 0)}</div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                    <span>Email: {review.user?.email || review.reviewerEmail || "N/A"}</span>
                    <span>Created: {new Date(review.createdAt).toLocaleString()}</span>
                    {review.reviewedAt ? (
                      <span>Reviewed: {new Date(review.reviewedAt).toLocaleString()}</span>
                    ) : (
